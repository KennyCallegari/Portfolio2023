 ...
 */
export const useInitialRootStore = (callback: (_: RootStore) => void | Promise<void>) => {
  const rootStore = useStores()
...
*/
export async function storeAssets(rootStore: RootStore) {
  ...
  Asset
    ...
    .then(
      (assets) => {
        ...
        rootStore?.assetsStore?.setProp("projectAssets", projectAssets)
      }
    )
}